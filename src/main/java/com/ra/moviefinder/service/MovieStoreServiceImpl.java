...
import com.proto.moviecontroller.MovieStoreRequest;
import com.proto.moviecontroller.MovieStoreResponse;
import com.proto.moviecontroller.MovieStoreServiceGrpc;
...
public class MovieStoreServiceImpl extends MovieStoreServiceGrpc.MovieStoreServiceImplBase {
	...
	public void getMovies(MovieStoreRequest request, StreamObserver<MovieStoreResponse> responseObserver) {
		...
		movies.stream().filter(movie -> movie.getGenre().equals(request.getGenre())).collect(Collectors.toList()).forEach(movie -> {
